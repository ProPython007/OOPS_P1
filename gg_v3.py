...
class Game():
    ...
    def start(self):
        ...
        self.canvas = Canvas(self.root, width=Game.WIDTH + 100, height=Game.HEIGHT)
        self.v_line = self.canvas.create_line(Game.WIDTH, 0, Game.WIDTH, Game.HEIGHT, fill='black')
        self.h_line = self.canvas.create_line(2, 2, Game.WIDTH + 100, 2, fill='black')
        self.canvas.pack()
    ...
    def remove_complete_lines(self):
        ...
        # print(all_boxes)
        all_boxes_coords = {k : v for k, v in zip(all_boxes, [self.canvas.coords(box)[3] for box in all_boxes if self.canvas.coords(box)[0] < Game.WIDTH])}
        if 1 in all_boxes_coords.keys():
            del all_boxes_coords[1]
        # print('all_b_c:', all_boxes_coords)
        ...
        # print((Game.WIDTH/Shape.BOX_SIZE))
        # print([(k, v) for k, v in counter.items()])
        # print()
        complete_lines = [k for k, v in counter.items() if v == ((Game.WIDTH/Shape.BOX_SIZE) - 1)]
        ...
        for point in shape[2:]:
            if shape[1] == 2:
                nbox = canvas.create_rectangle(
                    point[0] * Shape.BOX_SIZE + Game.WIDTH + 30,
                    point[1] * Shape.BOX_SIZE + Game.HEIGHT//2 - 25,
                    point[0] * Shape.BOX_SIZE + Shape.BOX_SIZE + Game.WIDTH + 30,
                    point[1] * Shape.BOX_SIZE + Shape.BOX_SIZE + Game.HEIGHT//2 - 25,
                    fill=shape[0])
            elif shape[1] == 4:
                nbox = canvas.create_rectangle(
                    point[0] * Shape.BOX_SIZE + Game.WIDTH + 10,
                    point[1] * Shape.BOX_SIZE + Game.HEIGHT//2 - 25,
                    point[0] * Shape.BOX_SIZE + Shape.BOX_SIZE + Game.WIDTH + 10,
                    point[1] * Shape.BOX_SIZE + Shape.BOX_SIZE + Game.HEIGHT//2 - 25,
                    fill=shape[0])
            else:
                nbox = canvas.create_rectangle(
                    point[0] * Shape.BOX_SIZE + Game.WIDTH + 20,
                    point[1] * Shape.BOX_SIZE + Game.HEIGHT//2 - 25,
                    point[0] * Shape.BOX_SIZE + Shape.BOX_SIZE + Game.WIDTH + 20,
                    point[1] * Shape.BOX_SIZE + Shape.BOX_SIZE + Game.HEIGHT//2 - 25,
                    fill=shape[0])
